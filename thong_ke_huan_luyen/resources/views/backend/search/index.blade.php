        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('search.index') }}" method="GET" class="row g-3 mb-4">
                    <div class="col-md-6">
                        <input type="text" name="q" class="form-control" placeholder="Nhập tên, mã quân nhân, chức vụ..." value="{{ request('q') }}">
                    </div>
                    <div class="col-md-4">
                                        ...
                                        <td>
                                            @foreach($soldier->weapons as $weapon)
                                                @php
                                                    ...
                                                    if($weapon->infantry_shovel) $details[] = "Xẻng: $weapon->infantry_shovel";
                                                    if($weapon->infantry_pickaxe) $details[] = "Cuốc: $weapon->infantry_pickaxe";
                                                    if($weapon->grenade) $details[] = "Lựu đạn: $weapon->grenade";
                                                @endphp
                                                @foreach($details as $detail)
                                                    <span class="badge badge-info mb-1">{{ $detail }}</span>
                                                @endforeach
                                                @if(count($details) == 0 && $soldier->weapons->count() > 0)
                                                    <span class="badge badge-secondary">Có trang bị khác</span>
                                                @endif
                                            @endforeach
                                        </td>
                                        <td>
                                            <a href="{{ route('soldiers.show', $soldier->id) }}" class="btn btn-sm btn-info">
